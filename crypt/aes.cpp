//#include "../common/stdafx.h"
 ...
 /*
   ...
   160, 192, 224 and 256 bits (16, 20, 24, 28 & 32 bytes) by Brian Gladman.
...
cf_dec c_name(my_encrypt)(const byte in_blk[], byte out_blk[], const c_name(aes) *cx)
{   word        locals(b0, b1);
    ...
    state_in((cx->Nrnd & 1 ? b1 : b0), in_blk, kp); kp += nc;

/*    switch(cx->Nrnd)
    {
    case 14:    round(fwd_rnd,  b1, b0, kp         ); kp += nc;
	case 13:    round(fwd_rnd,  b0, b1, kp         ); kp += nc;
    case 12:    round(fwd_rnd,  b1, b0, kp         ); kp += nc;
	case 11:    round(fwd_rnd,  b0, b1, kp         ); kp += nc;*/
  //  case 10:   
...
cf_dec c_name(my_decrypt)(const byte in_blk[], byte out_blk[], const c_name(aes) *cx)
{   word        locals(b0, b1);
    ...
    switch(cx->Nrnd)
    {
    case 14:    round(inv_rnd,  b1, b0, kp         ); kp += nc;
    case 13:    round(inv_rnd,  b0, b1, kp         ); kp += nc;
    case 12:    round(inv_rnd,  b1, b0, kp         ); kp += nc;
    case 11:    round(inv_rnd,  b0, b1, kp         ); kp += nc;
...
/***********************************************************************************************
								DoAes256:256位Aes加密函数
************************************************************************************************/

int DoAes256(UInt8* pIn,int nInlen,UInt8* pOut,int nOutLen,UInt8* pKey/*=32字节数组*/,int bEncypt)
{
	int nRound;
	int nRoundEx;
	int i,j,k;
	...
	if (bEncypt!=AES_EN && bEncypt!=AES_DE)
	{
		return 0;
	}
	if (nInlen<=0 || nOutLen<=0 || nOutLen<nInlen)
	{
		return 0;
	}	
	nRound=nInlen/16;
	nRoundEx=(nInlen%16)>0?1:0;
	if (nRoundEx>0)
	{
		if (nOutLen<(nRound+nRoundEx)*16)
		{
			return 0;
		}
	}
	if (bEncypt==AES_EN)
	{
		f_set_key(&alg, pKey, 16, enc);
		for(k=0;k<nRound+nRoundEx;k++)
		{   
			if(k==nRound && nRoundEx>0)
			{
				for (i=k*16,j=0;i<nInlen;i++,j++)
				{
					pBuff[j]=pIn[i];
				}
				for (;j<16;j++)
				...
				f_encrypt(&alg,pBuff,&pOut[16*k]);
	
			}
			else
			{
				f_encrypt(&alg, &pIn[16*k], &pOut[16*k]);
			}
		}	

	}	
	else if (bEncypt==AES_DE)
	{
		f_set_key(&alg, pKey, 16, dec);
		for(k=0;k<nRound+nRoundEx;k++)
		{   
			if(k==nRound && nRoundEx>0)
			{
				...
				f_decrypt(&alg,pBuff,&pOut[16*k]);
			}
			else
			{
				f_decrypt(&alg, &pIn[16*k], &pOut[16*k]);
			}
		}
	}
	return k*16;
