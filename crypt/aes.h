...
typedef UInt8   byte;           /* must be an 8-bit storage unit */
typedef UInt32   word;           /* must be a 32-bit storage unit */
typedef Int16           aes_ret;        /* function return value         */
...
#define f_encrypt(a,b,c)    my_encrypt((b),(c),(a))
#define f_decrypt(a,b,c)    my_decrypt((b),(c),(a))
#define f_set_blk(a,b)      set_blk((b),(a))

#define AES_EN 0
#define AES_DE 1

int DoAes256(UInt8* pIn,int nInlen,UInt8* pOut,int nOutLen,UInt8* pKey,int bEncypt);
...
#endif
    aes_ret set_key(const UInt8 key[], const word n_bytes, const aes_key f)
            { return c_name(set_key)(key, n_bytes, f, &cx); }
    aes_ret my_encrypt(const UInt8 in_blk[], UInt8 out_blk[]) const
            { return c_name(my_encrypt)(in_blk, out_blk, &cx); }
    aes_ret my_decrypt(const UInt8 in_blk[], UInt8 out_blk[]) const
            { return c_name(my_decrypt)(in_blk, out_blk, &cx); }
};
