...
#ifndef _INSPECTOR_MGR_H_
#define _INSPECTOR_MGR_H_

#include "common/BDScModuleBase.h"
#include "config/BDOptions.h"

#define  INSPECTOR_INFO_V(str) LOG4CXX_INFO(log4cxx::Logger::getLogger("INSPECTOR"), str)
#define  INSPECTOR_DEBUG_V(str) LOG4CXX_DEBUG(log4cxx::Logger::getLogger("INSPECTOR"), str)
#define  INSPECTOR_WARN_V(str) LOG4CXX_WARN(log4cxx::Logger::getLogger("INSPECTOR"), str)
#define  INSPECTOR_ERROR_V(str) LOG4CXX_ERROR(log4cxx::Logger::getLogger("INSPECTOR"), str)

#define  INSPECTOR_INFO_S(str) LOG4CXX_INFO(log4cxx::Logger::getLogger("INSPECTOR"), #str)
#define  INSPECTOR_DEBUG_S(str) LOG4CXX_DEBUG(log4cxx::Logger::getLogger("INSPECTOR"), #str)
#define  INSPECTOR_WARN_S(str) LOG4CXX_WARN(log4cxx::Logger::getLogger("INSPECTOR"), #str)
#define  INSPECTOR_ERROR_S(str) LOG4CXX_ERROR(log4cxx::Logger::getLogger("INSPECTOR"), #str)

//配置文件结构
typedef struct BDInspectorConfigEx {
    char chLog4File[100];
    int nModuleId;
...
}tag_inspector_config_t;

//线程池结构
typedef struct BDInspectorThreadPoolEx {
	pthread_t t_id;
	unsigned int t_num;
	bool b_state;
}tag_inspector_threadpool_t;

//INSPECTOR插件策略结构
typedef struct BDInspectorPolicyEx {
	BDInspectorPolicyEx() {
		strTaskId     = "";
		bOnce         = false;
		strTimePolocy = "";
		strIP         = "";
		vec_options.clear();
		for (int i=0; i < 5; i++) { // 0:ip连通性 1:cpu使用率 2:内存使用率 3:磁盘使用率 4:服务状态
...
}tag_inspector_policy_t;

class CInspectorMgr:public CScModuleBase {
	
public:
	CInspectorMgr(const string &strConfigName);
	virtual ~CInspectorMgr(void); 
...
private:
    ...
    static void* OnInspectorHandle(void *arg);     //ping处理
...
private:
	...
	vector<tag_inspector_policy_t> m_vecPolicy;           // 策略(任务)队列
	vector<tag_inspector_threadpool_t> m_vecThreadPool;   // 线程池
	
	int m_pthreadnum;            // 线程池中预创建线程的数量
	int m_pthreadnum_alive;      // 线程池中活着的线程数量 
	
public:
	tag_inspector_config_t m_inspectorConfig;

};

#endif //_INSPECTOR_MGR_H_
