...
#ifndef _WEAKPASSWD_MGR_H_
#define _WEAKPASSWD_MGR_H_

#include "../../common/BDScModuleBase.h"
#include "../../config/BDOptions.h"


#define  WEAKPASSWD_INFO_V(str) LOG4CXX_INFO(log4cxx::Logger::getLogger("WEAKPASSWD"), str)
#define  WEAKPASSWD_DEBUG_V(str) LOG4CXX_DEBUG(log4cxx::Logger::getLogger("WEAKPASSWD"), str)
#define  WEAKPASSWD_WARN_V(str) LOG4CXX_WARN(log4cxx::Logger::getLogger("WEAKPASSWD"), str)
#define  WEAKPASSWD_ERROR_V(str) LOG4CXX_ERROR(log4cxx::Logger::getLogger("WEAKPASSWD"), str)

#define  WEAKPASSWD_INFO_S(str) LOG4CXX_INFO(log4cxx::Logger::getLogger("WEAKPASSWD"), #str)
#define  WEAKPASSWD_DEBUG_S(str) LOG4CXX_DEBUG(log4cxx::Logger::getLogger("WEAKPASSWD"), #str)
#define  WEAKPASSWD_WARN_S(str) LOG4CXX_WARN(log4cxx::Logger::getLogger("WEAKPASSWD"), #str)
#define  WEAKPASSWD_ERROR_S(str) LOG4CXX_ERROR(log4cxx::Logger::getLogger("WEAKPASSWD"), #str)


//配置文件结构
typedef struct BDWeakpasswdConfigEx {
    char chLog4File[100];
    int nModuleId;
...
}tag_weakpasswd_config_t;

//线程池结构
typedef struct BDWeakpasswdThreadPoolEx {
	pthread_t t_id;
	unsigned int t_num;
	bool b_state;
}tag_weakpasswd_threadpool_t;

//WEAKPASSWD插件策略结构
typedef struct BDWeakpasswdPolicyEx {
    BDWeakpasswdPolicyEx() {
        strIP       = "";
        bFinished   = false;
        bReported   = false;
        strResult   = "";
    ...
    bool bFinished;
    bool bReported;
    string strResult;    // weakpasswd
}tag_weakpasswd_policy_t;


class CWeakpasswdMgr:public CScModuleBase {
	
public:
	CWeakpasswdMgr(const string &strConfigName);
	virtual ~CWeakpasswdMgr(void); 
...
private:
    ...
    static void* OnWeakpasswdHandle(void *arg);     //weakpasswd处理
...
private:
	...
	vector<tag_weakpasswd_policy_t> m_vecPolicy;           // 策略(任务)队列
	vector<tag_weakpasswd_threadpool_t> m_vecThreadPool;   // 线程池
	
	int m_pthreadnum;            // 线程池中预创建线程的数量
	int m_pthreadnum_alive;      // 线程池中活着的线程数量 
	
public:
	tag_weakpasswd_config_t m_weakpasswdConfig;

};

#endif //_WEAKPASSWD_MGR_H_
