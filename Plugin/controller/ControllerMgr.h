...
#ifndef _CONTROLLER_MGR_H_
#define _CONTROLLER_MGR_H_
...
//线程池结构
typedef struct BDControllerThreadPoolEx {
	pthread_t t_id;
	unsigned int t_num;
	bool b_state;
}tag_controller_threadpool_t;

//CONTROLLER插件策略结构
typedef struct BDControllerPolicyEx {
	BDControllerPolicyEx() {
		vec_options.clear();
...
}tag_controller_policy_t;

class CController:public CScModuleBase {
	
public:
	CController(const string &strConfigName);
...
private:
    ...
    static void* OnControllerHandle(void *arg);     //管控处理
...
private:
	...
	vector<tag_controller_policy_t> m_vecPolicy;           // 策略(任务)队列
	vector<tag_controller_threadpool_t> m_vecThreadPool;   // 线程池
