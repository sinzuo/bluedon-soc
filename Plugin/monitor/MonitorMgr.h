...
typedef struct BDMonitorPolicyEx {
	BDMonitorPolicyEx() {
		vec_options.clear();
        //为和inspector插件中子任务序号保持一致，这里子任务序号不使用0
        for (int i=1; i <= 7; i++) {
            //1:cpu使用率 2:内存使用率 3:磁盘使用率 4:服务状态 5：网关流量信息 6：安装列表信息 7：系统信息
...
private:
    ...
    void* GetSingleMointorInfo(CMonitorMgr *cThis,vector<string> v_options,int thread_num);
...
private:
	...
	vector<tag_monitor_policy_t> m_vecPolicy;           // 策略(任务)队列
	vector<tag_monitor_threadpool_t> m_vecThreadPool;   // 线程池
