...
#define GSA_LOCALE_DIR "/tmp"
#define GSA_DATA_DIR  "/tmp"
...
//TODO modified by fany@chinabluedon.cn
...
buffer_languages_xml(GString *buffer) {
    ...
    while (langs_list) {
        ...
        lang_name = (gchar *)g_hash_table_lookup(language_names, lang_code);

        if (lang_name == NULL) {
            lang_name = lang_code;
        }

        native_name = (gchar *)g_hash_table_lookup(native_language_names, lang_code);
