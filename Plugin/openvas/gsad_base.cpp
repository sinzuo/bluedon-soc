...
// add by fany@chinabluedon.cn @20170807 for bluedon
#define GSAD_VERSION "6.0.11"
...
xsl_transform_with_stylesheet(const char *xml_text,
    ...
    g_file_set_contents("/tmp/fanyi.xsl", xml_text, strlen(xml_text), &error);

    //g_warning ("fanyi content_file:%s, xml_text: [%s], error: [%s]", content_file, xml_text, error);
    if (error) {
...
params_append_bin(params_t *params, const char *name, const char *chunk_data,
    ...
    if (param == NULL) {
        ...
        value = (char *)g_malloc0(chunk_size + 1);
