...
#ifndef _NMAPSCAN_MGR_H_
#define _NMAPSCAN_MGR_H_

#include "common/BDScModuleBase.h"
#include "config/BDOptions.h"

#define  NMAPSCAN_INFO_V(str) LOG4CXX_INFO(log4cxx::Logger::getLogger("NMAPSCAN"), str)
#define  NMAPSCAN_DEBUG_V(str) LOG4CXX_DEBUG(log4cxx::Logger::getLogger("NMAPSCAN"), str)
#define  NMAPSCAN_WARN_V(str) LOG4CXX_WARN(log4cxx::Logger::getLogger("NMAPSCAN"), str)
#define  NMAPSCAN_ERROR_V(str) LOG4CXX_ERROR(log4cxx::Logger::getLogger("NMAPSCAN"), str)

#define  NMAPSCAN_INFO_S(str) LOG4CXX_INFO(log4cxx::Logger::getLogger("NMAPSCAN"), #str)
#define  NMAPSCAN_DEBUG_S(str) LOG4CXX_DEBUG(log4cxx::Logger::getLogger("NMAPSCAN"), #str)
#define  NMAPSCAN_WARN_S(str) LOG4CXX_WARN(log4cxx::Logger::getLogger("NMAPSCAN"), #str)
#define  NMAPSCAN_ERROR_S(str) LOG4CXX_ERROR(log4cxx::Logger::getLogger("NMAPSCAN"), #str)

//配置文件结构
typedef struct BDNmapscanConfigEx {
    char chLog4File[100];
    int nModuleId;
...
}tag_nmapscan_config_t;

//线程池结构
typedef struct BDNmapscanThreadPoolEx {
	pthread_t t_id;
	unsigned int t_num;
	bool b_state;
}tag_nmapscan_threadpool_t;

//nmapscan插件策略结构
typedef struct BDNmapscanPolicyEx {
//    BDNmapscanPolicyEx() {
//		vec_options.clear();
...
//	map<int,string> m_mapResult;         // 任务结果字符串(细化至子任务)
    /**/
    string strIP;    // IP
    string  nResult; // nmap 结果
    bool nfinished;    // 是否完成扫描
    /**/

}tag_nmapscan_policy_t;

class CNmapscanMgr:public CScModuleBase {
	
public:
    CNmapscanMgr(const string &strConfigName);
    virtual ~CNmapscanMgr(void);
...
private:
    ...
    static void* OnNmapscanHandle(void *arg);     //ping处理
...
private:
    ...
    vector<tag_nmapscan_policy_t> m_vecPolicy;           // 策略(任务)队列
    vector<tag_nmapscan_threadpool_t> m_vecThreadPool;   // 线程池
	
	int m_pthreadnum;            // 线程池中预创建线程的数量
	int m_pthreadnum_alive;      // 线程池中活着的线程数量 
	
public:
    tag_nmapscan_config_t m_nmapscanConfig;

};

#endif //_nmapscan_MGR_H_
